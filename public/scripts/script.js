...
$(window).bind('scroll', function () {
  ...
  else if ($(window).width() > 977) { // SWEETETH I THINK THIS 977 FIXED THAT ONE WACK SCREEN (WAS 992)
...
$(window).resize(function () {
  if ($(window).width() < 977) { // SWEETETH I THINK THIS 977 FIXED THAT ONE WACK SCREEN (WAS 992)
    $('.navbar').css({ "position": "fixed", "top": "0px" });
  }
...
amandaPhoto.onclick = () => {  // SWEETETH THIS MAKES THE FUFUS CHANGE PROPERLY ON PHONES/(HOPEFULLY) IPADS
  if ($(window).width() <= 992) {
    amandaPhoto.style.display = "none";
    amandaBio.style.display = "block";
  }
